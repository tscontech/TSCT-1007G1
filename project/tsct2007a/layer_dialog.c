...
void EmergencyDialogShow(ChargeStep step)
{
	...
	switch (step)
	{
		...
		case STEP_REQ_CLOSE:
		case STEP_CHARGE:
			ituTextSetString(sEmergencyDialogText1, STR_TOUCHKEY_EMERGENCY_DIALOG_1);
			ituTextSetString(sEmergencyDialogText2, STR_TOUCHKEY_EMERGENCY_DIALOG_2);
...
void WhmErrorDialogTouchkeyListener(bool longPress)
{
	OkDialogHide(0);
}
...
{	
	...
	setPrivilegedTouchKeyListener(NULL);
}
