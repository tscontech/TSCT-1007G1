...
//Power On PLC, RFID, EPT, IDA
bool plcPowerInitialized = false;
bool plcPowerState = false;
...
//24.11.25 확실한 Dequeue를 위해 추가 JGLEE
bool ReadQueueDequeueEnsure(ReadQueue *q, uint8_t* buffer)
{
	if (ReadQueueIsEmpty(q)) {
		// printf("[COMMON] Read queue is empty\n");
		*buffer = 0;
...
// Power contorl for 1007G1
bool powerControlPLC(bool power)
{
	if(!plcPowerInitialized)
	{
		ithGpioSetMode(GPIO_PLC_PWR_RELAY_CTL, ITH_GPIO_MODE0);
		ithGpioSetOut(GPIO_PLC_PWR_RELAY_CTL);
		plcPowerInitialized = true;
	}
	if(plcPowerState != power)
	{
		if(power)	ithGpioSet(GPIO_PLC_PWR_RELAY_CTL);
		else		ithGpioClear(GPIO_PLC_PWR_RELAY_CTL);

		plcPowerState = power;
	}

	return plcPowerState;
}