...
void MakeDataCmd_Auth(void)
{
    ...
    // strcat(Tx_Msg.Payload[0].property_contants, NULL);
	// memcpy(Tx_Msg.Payload[0].property_contants, "1110060507020004",sizeof("1110060507020004"));
		...
		for(int i=0;i<=Rx_Msg.Payload[0].subPayload_len;i++){
			if(!strcmp(Rx_Msg.Payload[0].sub_Payload[i].property_name, "status")){	// Allow
				if(!strcmp(Rx_Msg.Payload[0].sub_Payload[i].property_contants, "Accepted"))
				{
					...
					// CsConfigVal.bCpStat[bDevChannel+1] = CP_STATUS_CODE_PREPARE;
				...
				// shmDataIfInfo.card_auth = CARD_AUTH_FAILD;
				// printf("[DataProcCmd_Auth] Card Auth Fail\r\n");
			}
		}
	}
	else	printf("[DataProcCmd_Auth] Non Check OK idTagInfo\r\n");
	
	// if(CsConfigVal.bReqAuthNo == 2 && shmDataIfInfo.card_auth == CARD_AUTH_OK){
...
void MakeDataCmd_Boot(void)
{
	...
	memset(Tx_Msg.Payload[0].property_contants, 0x00, sizeof(Tx_Msg.Payload[0].property_contants));
	memcpy(Tx_Msg.Payload[0].property_contants, "TSCT",sizeof("TSCT"));
	Tx_Msg.Payload[0].data_type = TYPE_CODE_STR;
...
void DataProcCmd_Boot(void)
{
	...
	for(int i=0;i<=Rx_Msg.Payload_len ;i++)
	{
		...
		else if(!strcmp(Rx_Msg.Payload[i].property_name, "currentTime"))
		{
			...
			+ (((Rx_Msg.Payload[i].property_contants[6] - '0') % 10) * 1) - 1;  // month range is 0-11		
...
void MakeDataCmd_DataTransCp(TSCT_ERR_CODE code_no)
{
	char* orgBuf;
	...
	switch (code_no)
	{
		case TSCT_ERR_CODE_EMG:
			...
			memset(Tx_Msg.Payload[0].property_contants, 0x00, sizeof(Tx_Msg.Payload[0].property_contants));
			// memcpy(Tx_Msg.Payload[0].property_contants, "211.43.203.70",sizeof("211.43.203.70"));
			memcpy(Tx_Msg.Payload[0].property_contants, "www.tscontech.com",sizeof("www.tscontech.com"));
			Tx_Msg.Payload[0].data_type = TYPE_CODE_STR;

			memset(Tx_Msg.Payload[1].property_name, 0x00, sizeof(Tx_Msg.Payload[1].property_name));
			memcpy(Tx_Msg.Payload[1].property_name, "data",sizeof("data"));

			memset(Tx_Msg.Payload[1].property_contants, 0x00, sizeof(Tx_Msg.Payload[1].property_contants));
			memcpy(Tx_Msg.Payload[1].property_contants, "Input the EMG Button",sizeof("Input the EMG Button"));

			Tx_Msg.Payload[1].data_type = TYPE_CODE_STR;

		break;

		case TSCT_ERR_CODE_BATDATA:
		case TSCT_ERR_CODE_SECCBATDATA:

			if(vasLastDataCnt > 0) {
				dataInfoCnt = vasLastDataCnt;	
			}
			else {
				dataInfoCnt = CsConfigVal.batInfo_cfgCnt;	
			}
			...
			memset(Tx_Msg.Payload[0].property_contants, 0x00, sizeof(Tx_Msg.Payload[0].property_contants));
			// memcpy(Tx_Msg.Payload[0].property_contants, "211.43.203.70",sizeof("211.43.203.70"));
			memcpy(Tx_Msg.Payload[0].property_contants, "kr.or.keco",sizeof("kr.or.keco"));
			Tx_Msg.Payload[0].data_type = TYPE_CODE_STR;
			...
			Tx_Msg.Payload[2].data_type = TYPE_CODE_OBJ_STR;

			Tx_Msg.Payload[2].subPayload_len = 2;
			...
			memcpy(Tx_Msg.Payload[2].sub_Payload[0].property_name, "infoCnt", sizeof("infoCnt"));

			memset(Tx_Msg.Payload[2].sub_Payload[0].property_contants, 0x00, sizeof(Tx_Msg.Payload[2].sub_Payload[0].property_contants));
			// memcpy(Tx_Msg.Payload[2].sub_Payload[0].property_contants, "infoCnt", sizeof("infoCnt"));

			sprintf(Tx_Msg.Payload[2].sub_Payload[0].property_contants, "%d", dataInfoCnt);

			Tx_Msg.Payload[2].sub_Payload[0].data_type = TYPE_CODE_STR;

			memset(Tx_Msg.Payload[2].sub_Payload[1].property_name, 0x00, sizeof(Tx_Msg.Payload[2].sub_Payload[1].property_name));
			memcpy(Tx_Msg.Payload[2].sub_Payload[1].property_name, "batteryDataSet", sizeof("batteryDataSet"));

			Tx_Msg.Payload[2].sub_Payload[1].data_type = TYPE_CODE_ARR;
			Tx_Msg.Payload[2].sub_Payload[1].subPayload_len = 2;
			Tx_Msg.Payload[2].sub_Payload[1].subPayloadArrLen = dataInfoCnt;
			Tx_Msg.Payload[2].sub_Payload[1].sub_Payload = &(tx_sub_Payload[5]);

			for(int i=0; i<dataInfoCnt; i++) {
				for(int j=0; j<2; j++) {
					memset(Tx_Msg.Payload[2].sub_Payload[1].sub_Payload[j+(2*i)].property_name, 0x00, sizeof(Tx_Msg.Payload[2].sub_Payload[1].sub_Payload[j+(8*i)].property_name));
					memset(Tx_Msg.Payload[2].sub_Payload[1].sub_Payload[j+(2*i)].property_contants, 0x00, sizeof(Tx_Msg.Payload[2].sub_Payload[1].sub_Payload[j+(8*i)].property_contants));
					
					memcpy(Tx_Msg.Payload[2].sub_Payload[1].sub_Payload[j+(2*i)].property_name, batDataNameEnc[j], strlen(batDataNameEnc[j]));

					Tx_Msg.Payload[2].sub_Payload[1].sub_Payload[j+(2*i)].data_type = TYPE_CODE_STR;
				}
				...
				memcpy(Tx_Msg.Payload[2].sub_Payload[1].sub_Payload[0+(2*i)].property_contants, tmpStrBuf, strlen(tmpStrBuf));
				
				encodeBuf = malloc(1024);
				
				Curl_base64_encode(orgBuf, (size_t)(orgBytes), &encodeBuf, &bytes_encode);
				// Curl_base64_encode(orgBuf, NULL, &encodeBuf, NULL);

				// sprintf(Tx_Msg.Payload[2].sub_Payload[0].property_contants, "%s", encodeBuf);

				// printf(" / Check size : %d\r\n\r\n", bytes_encode);

				memcpy(Tx_Msg.Payload[2].sub_Payload[1].sub_Payload[1+(2*i)].property_contants, encodeBuf, strlen(encodeBuf)+1);
			...
			printf("[MakeDataCmd_DataTransCp] IDA Battery Data Setting OK\r\n");
...
void DataProcCmd_HB(void)
{
	...
	if(!strcmp(Rx_Msg.Payload[0].property_name, "currentTime")){
		...
		+ (((Rx_Msg.Payload[0].property_contants[6] - '0') % 10) * 1) - 1;		

		tmp_time.tm_mday = (((Rx_Msg.Payload[0].property_contants[8] - '0') % 10) * 10)	\
		+ (((Rx_Msg.Payload[0].property_contants[9] - '0') % 10) * 1);  //month range is 0-11 ????
...
void MakeDataCmd_MeterVal(void)
{
	...
	if(MeterValQ[0].Meterval_Flg != METER_VAL_TRIG_TYPE)
	{
		Tx_Msg.Payload_len = 3;
	...
	{	//Triggered MeterValue Must not have TransactionID
		Tx_Msg.Payload_len = 2;
		
		memset(Tx_Msg.Payload[1].property_name, 0x00, sizeof(Tx_Msg.Payload[2].property_name));
		memcpy(Tx_Msg.Payload[1].property_name, "meterValue",sizeof("meterValue"));

		Tx_Msg.Payload[1].data_type = TYPE_CODE_ARR;

		Tx_Msg.Payload[1].sub_Payload = tx_sub_Payload;
		Tx_Msg.Payload[1].subPayload_len = 1;
		...
		if(MeterValQ[0].Meterval_Flg == METER_VAL_CLOCK_TYPE)
			sprintf((Tx_Msg.Payload[1].sub_Payload)->property_contants, "{\"timestamp\":\"%s\",\"sampledValue\":[{\"value\":\"%lu\",\"measurand\":\"Energy.Active.Import.Register\",\"context\":\"Sample.Clock\",\"unit\":\"Wh\"}]}", MeterValQ[0].Time_Stamp, MeterValQ[0].Sampled_Val);
		else if(MeterValQ[0].Meterval_Flg == METER_VAL_SAMP_TYPE)
			sprintf((Tx_Msg.Payload[1].sub_Payload)->property_contants, "{\"timestamp\":\"%s\",\"sampledValue\":[{\"value\":\"%lu\",\"measurand\":\"Energy.Active.Import.Register\",\"context\":\"Sample.Periodic\",\"unit\":\"Wh\"}]}", MeterValQ[0].Time_Stamp, MeterValQ[0].Sampled_Val);
		else if(MeterValQ[0].Meterval_Flg == METER_VAL_TRIG_TYPE)
			sprintf((Tx_Msg.Payload[1].sub_Payload)->property_contants, "{\"timestamp\":\"%s\",\"sampledValue\":[{\"value\":\"%lu\",\"measurand\":\"Energy.Active.Import.Register\",\"context\":\"Trigger\",\"unit\":\"Wh\"}]}", MeterValQ[0].Time_Stamp, MeterValQ[0].Sampled_Val);
	}
}
...
void MakeDataCmd_StartTs(void)
{
	...
	// }
	sprintf(temp_buf,"%lu", startTsQ.MeterStart_Val);
    ...
    memset(Tx_Msg.Payload[3].property_name, 0x00, sizeof(Tx_Msg.Payload[1].property_name));
...
void DataProcCmd_StartTs(void)
{
	for(int i=0;i<=Rx_Msg.Payload_len ;i++){
		if(!strcmp(Rx_Msg.Payload[i].property_name, "idTagInfo")){
			...
			CsConfigVal.bTrId[bDevChannel+1] = String2Integer(Rx_Msg.Payload[i].property_contants);
		}
	}
...
void MakeDataCmd_Stat(uint8_t cntrNo)
{
	...
	memset(Tx_Msg.Payload[1].property_name, 0x00, sizeof(Tx_Msg.Payload[1].property_name));
	memcpy(Tx_Msg.Payload[1].property_name, "errorCode",sizeof("errorCode"));	
	...
	memset(Tx_Msg.Payload[1].property_contants, 0x00, sizeof(Tx_Msg.Payload[1].property_contants));
	memcpy(Tx_Msg.Payload[1].property_contants, "NoError",sizeof("NoError"));
	Tx_Msg.Payload[1].data_type = TYPE_CODE_STR;
	...
	case CP_STATUS_CODE_PREPARE:
		// memset(Tx_Msg.Payload[0].property_contants, NULL, sizeof(Tx_Msg.Payload[0].property_contants));
		// memcpy(Tx_Msg.Payload[0].property_contants, "0",sizeof("0"));
	...
	case CP_STATUS_CODE_SUSPENDEDEV:
		...
		memcpy(Tx_Msg.Payload[3].property_name, "info",sizeof("info"));

		memset(Tx_Msg.Payload[3].property_contants, 0x00, sizeof(Tx_Msg.Payload[3].property_contants));
		memcpy(Tx_Msg.Payload[3].property_contants, "EV side disconnected",sizeof("EV side disconnected"));
		Tx_Msg.Payload[3].data_type = TYPE_CODE_STR;
	...
	case CP_STATUS_CODE_FAULT:
		...
		// memset(Tx_Msg.Payload[1].property_contants, NULL, sizeof(Tx_Msg.Payload[1].property_contants));
		// memcpy(Tx_Msg.Payload[1].property_contants, "7",sizeof("7"));
...
void MakeDataCmd_StopTs(void)
{
	...
	sprintf(temp_buf,"%llu",StopTsConfig.TrId);
	memcpy(Tx_Msg.Payload[0].property_contants, temp_buf,sizeof(temp_buf));
	Tx_Msg.Payload[0].data_type = TYPE_CODE_INT;
	...
	// 	temp_buf32 += (uint32_t)(shmDataAppInfo.eqp_watt[i] << ((3-i)*8));
	// }
	sprintf(temp_buf,"%lu",StopTsConfig.MeterStop_Val);
	...
	else if(StopTsConfig.Stop_Reason == STOP_REASON_DEAUTH)
	{
		memset(Tx_Msg.Payload[Tx_Msg.Payload_len].property_name, 0x00, sizeof(Tx_Msg.Payload[Tx_Msg.Payload_len].property_name));
		memcpy(Tx_Msg.Payload[Tx_Msg.Payload_len].property_name, "reason",sizeof("reason"));

		memset(Tx_Msg.Payload[Tx_Msg.Payload_len].property_contants, 0x00, sizeof(Tx_Msg.Payload[Tx_Msg.Payload_len].property_contants));
		memcpy(Tx_Msg.Payload[Tx_Msg.Payload_len].property_contants, "DeAuthorized",sizeof("DeAuthorized"));
