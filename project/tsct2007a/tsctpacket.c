...
static short CopyEpqData(int ch)
{
	...
	if(CstGetEpqStatus(TSCT_SELF_TEST)) // 0x05 waiting mode ; 0x09 charging mode ; 0x11 test mode ; 0x21 error mode ; 
											//0x41 charging complete ; 0x81 emergency mode 
	{
		shmDataAppInfo.eqp_mode[1] = 0x11;
	}
	else if(CstGetEpqStatus(TSCT_EMG_SW)) 
	{
		shmDataAppInfo.eqp_mode[1] = 0x81;
	}
	else if(CstGetEpqStatus(TSCT_SELF_TEST)) 
	{
